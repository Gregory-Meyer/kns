...
static FORCEINLINE int32_t atomic_incr32(atomic32_t *val) {
  return __atomic_fetch_add(val, 1, __ATOMIC_RELAXED) + 1;
}
static FORCEINLINE int32_t atomic_decr32(atomic32_t *val) {
  return __atomic_fetch_add(val, -1, __ATOMIC_RELAXED) - 1;
}
static FORCEINLINE int32_t atomic_add32(atomic32_t *val, int32_t add) {
  return __atomic_fetch_add(val, add, __ATOMIC_RELAXED) + add;
}
static FORCEINLINE int atomic_cas32_acquire(atomic32_t *dst, int32_t val,
                                            int32_t ref) {
  return __atomic_compare_exchange_n(dst, &ref, val, 1, __ATOMIC_ACQUIRE,
                                     __ATOMIC_RELAXED);
}
static FORCEINLINE void atomic_store32_release(atomic32_t *dst, int32_t val) {
...
static FORCEINLINE void *atomic_exchange_ptr_acquire(atomicptr_t *dst,
                                                     void *val) {
  return __atomic_exchange_n(dst, val, __ATOMIC_ACQUIRE);
}
static FORCEINLINE int atomic_cas_ptr(atomicptr_t *dst, void *val, void *ref) {
  return __atomic_compare_exchange_n(dst, &ref, val, 1, __ATOMIC_RELAXED,
                                     __ATOMIC_RELAXED);
}
...
//! Number of spans to release from thread cache to global cache (large multiple
//! spans)
...
static void _rpmalloc_span_mark_as_subspan_unless_master(span_t *master,
  ...
  if (subspan != master) {
    ...
    subspan->offset_from_master = (uint32_t)(
        (uintptr_t)pointer_diff(subspan, master) >> _memory_span_size_shift);
  ...
  if (_memory_page_size > _memory_span_size) {
    ...
    if (_memory_global_reserve_count >= span_count) {
      size_t reserve_count =
          (!heap->spans_reserved ? DEFAULT_SPAN_MAP_COUNT : span_count);
...
static void *_rpmalloc_span_initialize_new(heap_t *heap, span_t *span,
                                           uint32_t class_idx) {
  ...
  span->free_list_limit =
      free_list_partial_init(&heap->size_class[class_idx].free_list, &block,
  ...
  if (span->free_list_limit < span->block_count) {
    _rpmalloc_span_double_link_list_add(
        &heap->size_class[class_idx].partial_span, span);
...
static size_t _rpmalloc_global_cache_extract_spans(span_t **span,
  ...
  memcpy(span, cache->span + (cache->count - extract_count),
         sizeof(span_t *) * extract_count);
...
static span_t *_rpmalloc_heap_extract_new_span(heap_t *heap, size_t span_count,
                                               uint32_t class_idx) {
  span_t *span;
#if ENABLE_ADAPTIVE_THREAD_CACHE || ENABLE_STATISTICS
...
static heap_t *_rpmalloc_heap_allocate_new(void) {
  ...
  if (span_count > heap_span_count) {
    ...
    size_t reserve_count =
        (remain_count > DEFAULT_SPAN_MAP_COUNT ? DEFAULT_SPAN_MAP_COUNT
                                               : remain_count);
    span_t *remain_span =
  ...
  if (EXPECTED(span != 0)) {
    ...
    if (span->free_list) {
      ...
      span->free_list_limit += free_list_partial_init(
          &heap->size_class[class_idx].free_list, &block,
...
static void *_rpmalloc_allocate_large(heap_t *heap, size_t size) {
  ...
  // Find a span in one of the cache levels
  span_t *span =
      _rpmalloc_heap_extract_new_span(heap, span_count, SIZE_CLASS_LARGE);
  if (!span)
    return span;
...
static void _rpmalloc_deallocate_direct_small_or_medium(span_t *span,
  ...
  assert(heap->owner_thread == get_thread_id() || !heap->owner_thread ||
         heap->finalize);
...
static void _memory_heap_dump_statistics(heap_t *heap, void *file) {
  ...
  fprintf(file,
          "Spans  Current     Peak  PeakMiB  Cached  ToCacheMiB FromCacheMiB "
          "ToReserveMiB FromReserveMiB ToGlobalMiB FromGlobalMiB  MmapCalls\n");
  for (size_t iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
...
void rpmalloc_dump_statistics(void *file) {
#if ENABLE_STATISTICS
  // If you hit this assert, you still have active threads or forgot to finalize
  // some thread(s)
  assert(atomic_load32(&_memory_active_heaps) == 0);
  for (size_t list_idx = 0; list_idx < HEAP_ARRAY_SIZE; ++list_idx) {
    heap_t *heap = _memory_heaps[list_idx];
  ...
  size_t reserved_total =
      (size_t)atomic_load32(&_reserved_spans) * _memory_span_size;
  fprintf(file, "MappedMiB MappedOSMiB MappedPeakMiB MappedTotalMiB "
                "UnmappedTotalMiB ReservedTotalMiB\n");
